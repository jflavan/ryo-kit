# Cross-Runtime Symlink Architecture for Agents & Skills

**Date:** 2026-03-16
**Status:** Approved

## Problem

ryo-kit generates agents and skills into `.ryo/agents/` and `.ryo/skills/`, but each supported coding tool (Claude Code, Copilot, Cursor, Codex, Gemini CLI, Windsurf) expects files in its own directory. The current approach copies full file content into each runtime's directory, creating duplication and drift. Additionally, the Copilot integration writes to `.github/prompts/` (wrong location) — Copilot expects agents in `.github/agents/` and skills in `.github/skills/`.

Users who select multiple coding tools end up with duplicate files that can diverge after manual edits, with no single source of truth.

## Goals

1. Single source of truth for all agent and skill files
2. All six supported runtimes discover and use the files natively
3. Minimize symlinks — leverage auto-discovery where possible
4. Handle runtimes that don't support agents natively (config block fallback)
5. Handle Windsurf's fundamentally different format (inline copies)
6. Work on macOS, Linux, and Windows (with graceful fallback)

## Design

### Canonical File Locations

**Skills** move from `.ryo/skills/` to **`.agents/skills/*/SKILL.md`**.

This leverages the emerging cross-tool standard: Cursor, Codex, and Gemini CLI all auto-discover skills from `.agents/skills/`. Three of six runtimes get zero-configuration discovery.

Verified against tool documentation (March 2026):
- **Cursor:** `.agents/skills/` is a primary skills discovery path ([Cursor Docs: Agent Skills](https://cursor.com/docs/context/skills)). Also supports `.cursor/skills/`.
- **Codex:** `$CWD/.agents/skills/` is a scan location in the precedence chain ([OpenAI Codex: Agent Skills](https://developers.openai.com/codex/skills/)). Also scans parent directories up to repo root.
- **Gemini CLI:** `.agents/skills/` is an alias path with higher precedence than `.gemini/skills/` within the workspace tier ([Gemini CLI: Agent Skills](https://geminicli.com/docs/cli/skills/)).

**Agents** stay at **`.ryo/agents/*.agent.md`**.

There is no cross-tool standard for agent files. Claude Code and Copilot have native agent directories but with different naming (`.md` vs `.agent.md`). Codex uses TOML. The remaining three runtimes have no native agent directory at all.

### Skills: Per-Runtime Strategy

| Runtime | Strategy | Target Path | Detail |
|---|---|---|---|
| Claude Code | symlink (dir) | `.claude/skills/{name}/` → `../../.agents/skills/{name}/` | Directory symlink |
| Copilot | symlink (dir) | `.github/skills/{name}/` → `../../.agents/skills/{name}/` | Directory symlink |
| Cursor | auto-discovery | `.agents/skills/{name}/SKILL.md` | No action needed |
| Codex | auto-discovery | `.agents/skills/{name}/SKILL.md` | No action needed |
| Gemini CLI | auto-discovery | `.agents/skills/{name}/SKILL.md` | No action needed |
| Windsurf | copy (transform) | `.windsurf/rules/{name}.md` | Copy with Windsurf-specific frontmatter: replaces ryo-kit's `trigger` field (a description string) with Windsurf's `trigger: model_decision` (an enum value), and moves the ryo-kit `description` to the Windsurf `description` field. This migrates from the legacy `.windsurfrules` flat file to the modern `.windsurf/rules/` directory format (Windsurf Wave 8+). |

### Agents: Per-Runtime Strategy

| Runtime | Strategy | Target Path | Detail |
|---|---|---|---|
| Claude Code | symlink (file) | `.claude/agents/{name}.md` → `../../.ryo/agents/{name}.agent.md` | File symlink. Claude Code discovers `.md` files in `.claude/agents/`; the symlink name uses `.md` extension regardless of the target's `.agent.md` extension. |
| Copilot | symlink (file) | `.github/agents/{name}.agent.md` → `../../.ryo/agents/{name}.agent.md` | File symlink |
| Codex | generate | `.codex/agents/{name}.toml` | Transform AgentDefSchema YAML → Codex TOML format. Codex expects custom agents as standalone TOML files in `~/.codex/agents/` (personal) or `.codex/agents/` (project-scoped) with required fields: `name`, `description`, `developer_instructions`. |
| Cursor | config block | `AGENTS.md` | Upsert ryo-kit block with agent descriptions |
| Gemini CLI | config block | `GEMINI.md` | Upsert ryo-kit block with agent descriptions |
| Windsurf | config block | `AGENTS.md` | Upsert ryo-kit block (Windsurf auto-scopes by directory) |

### Naming Conventions

**Bootstrap/core skills** retain the `ryo-` prefix: `ryo-gen`, `ryo-help`, `ryo-add-agent`, `ryo-add-skill`, `ryo-evolve`, `ryo-retro`. These are ryo-kit's own skills.

**Generated skills** (from `/ryo-gen`) use unprefixed names: `implement`, `review`, `test`, `design`, etc. These are the user's framework skills.

Both follow the same directory pattern in `.agents/skills/`. The `ryo-` prefix is just a naming convention, not a structural difference.

### Codex Agent TOML Transform

Example of the generated TOML for a Codex agent, transformed from the canonical `.agent.md`:

```toml
# Generated by ryo-kit from .ryo/agents/builder.agent.md
name = "builder"
description = "Plans and implements code changes based on design documents and task specifications."

developer_instructions = """
# Builder

## Role
Plans and implements code changes. Reads design documents and task
specifications, produces implemented code, and hands off to the reviewer.

## Handoff Protocol
All code must compile, pass linting, and have test stubs before handoff.
"""
```

The `developer_instructions` field is assembled from the markdown body of the canonical `.agent.md` file. Ryo-kit-specific frontmatter fields (`responsibilities`, `handoff_to`, `gate`) are included as structured comments or embedded in the instructions text.

The sentinel comment `# Generated by ryo-kit from ...` is the detection mechanism for cleanup — `uninstall()` and `ryo sync` only delete `.codex/agents/*.toml` files that begin with this header.

### Frontmatter Compatibility

Skills use YAML frontmatter with these fields:

```yaml
---
name: implement          # Required. Must match directory name.
description: >           # Required. Describes what the skill does AND when to use it.
  Implements code changes based on design documents and task specifications.
trigger: "When the user needs to implement a feature"
agent: builder           # Optional. Which agent typically uses this skill.
inputs:
  - design documents
  - task specifications
outputs:
  - implemented code
runtimes:
  - claude-code
  - copilot
  - cursor
---
```

All runtimes require `name` + `description`. The ryo-kit-specific fields (`trigger`, `agent`, `inputs`, `outputs`, `runtimes`) are ignored by all runtimes — they skip unknown frontmatter keys. No schema changes needed.

Agent frontmatter similarly carries ryo-kit fields (`responsibilities`, `handoff_to`, `gate`) that Claude Code and Copilot ignore while reading the shared fields (`name`, `description`, `tools`).

### The `ryo sync` Command

New CLI command that creates/updates all runtime links and references.

**What it does:**
1. Reads `org-context.yaml` to determine active runtimes
2. Scans `.agents/skills/` for all skill directories
3. Scans `.ryo/agents/` for all agent files
4. For each active runtime, performs the appropriate action (symlink, generate, config block, or copy) per the tables above
5. Cleans up stale ryo-kit symlinks (only those pointing into `.agents/skills/` or `.ryo/agents/`) — user-created symlinks are never touched

**When it runs:**
- Explicitly: `ryo sync` (manual invocation)
- After `ryo gen`: automatically after installing bootstrap skills
- After `/ryo-gen` completes: the orchestrator skill instructs the AI to run `npx ryo-kit sync`
- After `ryo evolve`: re-sync after regeneration

**Symlink creation with Windows fallback:**
```
attempt fs.symlink() (relative path)
  → on success: done
  → on EPERM/ENOTSUP: fall back to copying file content
    → log warning: "Symlinks unavailable, using copies. Run ryo sync after edits to re-copy."
```

On Windows, directory symlinks use `fs.symlink(target, path, 'junction')` which works without elevation. File symlinks attempt `'file'` type first, falling back to copy.

**Error handling:**
- If `.agents/skills/` does not exist: create it (same as `scaffoldProjectDir` would).
- If `org-context.yaml` is missing: abort with a clear error ("Run `ryo init` first").
- If a config file (`AGENTS.md`, `GEMINI.md`, etc.) is read-only: warn and skip that runtime, continue with others.
- If a symlink target does not exist (canonical file was deleted): skip and warn, do not create a dangling symlink.
- Exit code 0 on success (even with warnings), exit code 1 on fatal errors (missing org context).

### `BaseRuntime` Interface Changes

```js
class BaseRuntime {
  constructor(projectDir) { ... }

  get name()       // string — runtime identifier
  get skillsDir()  // string|null — where this runtime reads skills (null if auto-discovery)
  get agentsDir()  // string|null — where this runtime reads agents (null if no native dir)
  get configFile() // string — main config file path

  // Install a skill. For symlink runtimes, creates a symlink from skillsDir
  // to the canonical path. For auto-discovery runtimes, this is a no-op.
  // For Windsurf, copies with frontmatter transform.
  async installSkill(skillName, canonicalSkillDir)

  // Install an agent. For symlink runtimes, creates a symlink from agentsDir.
  // For Codex, generates a TOML file. For config-block runtimes, upserts a
  // sentinel block in the config file with agent descriptions.
  // agentMeta: { name, role, description, responsibilities, handoff_to, gate, body }
  // where body is the markdown content below frontmatter.
  async installAgent(agentName, agentMeta)

  async updateConfig(contextRef)  // unchanged
  async uninstall()               // updated per runtime (see Changes to uninstall())
}
```

### Changes to `installSkillsForRuntimes()`

Currently writes skill content directly to each runtime's directory. Changes to:
1. Write skill content to `.agents/skills/{name}/SKILL.md` (canonical location)
2. Call `sync` to create symlinks/copies for each active runtime

The current name-derivation logic in `skill-writer.js` strips the `ryo-` prefix from template filenames (e.g., `ryo-gen.skill.md` → `gen`), then each runtime re-adds it (e.g., `ryo-gen/SKILL.md`). This changes: the canonical skill name retains the `ryo-` prefix for bootstrap/core skills. The name-derivation logic should produce `ryo-gen` (not `gen`) and write to `.agents/skills/ryo-gen/SKILL.md`. The `ryo-` prefix stripping in `skill-writer.js` lines 59-62 is removed.

### Changes to `updateConfig()`

The existing `updateConfig(contextRef)` method on each runtime writes a ryo-kit block to the runtime's config file (e.g., `CLAUDE.md`, `.cursorrules`, `GEMINI.md`). This behavior is unchanged for skill-related context references.

For agent references in runtimes without native agent directories (Cursor, Gemini CLI, Windsurf), the agent config block is written via the new `installAgent()` method to `AGENTS.md` or `GEMINI.md` — not via `updateConfig()`. This keeps the two concerns separate: `updateConfig()` handles general context references, `installAgent()` handles agent definitions.

For Cursor specifically, the `configFile` getter remains `.cursorrules` for backward-compatible context references via `updateConfig()`. Agent definitions use `installAgent()` which writes to `AGENTS.md` — a separate file. Both files may coexist; they serve different purposes.

### Changes to `uninstall()`

Each runtime's `uninstall()` method changes to match its new install strategy:
- **Claude Code, Copilot:** Remove symlinks from `.claude/skills/`, `.claude/agents/`, `.github/skills/`, `.github/agents/` (only ryo-kit-created symlinks targeting `.agents/skills/` or `.ryo/agents/`)
- **Codex:** Delete `.codex/agents/*.toml` files generated by ryo-kit
- **Cursor, Gemini CLI:** Remove ryo-kit sentinel blocks from `AGENTS.md` / `GEMINI.md`
- **Windsurf:** Delete `.windsurf/rules/` files that were copied by ryo-kit; remove ryo-kit sentinel blocks from `AGENTS.md`. The legacy `.windsurfrules` cleanup is preserved for migration.

### Changes to Generation Sub-Skills

The `skill-generation.skill.md` template changes its output path:
- **Before:** `.ryo/skills/{name}/SKILL.md`
- **After:** `.agents/skills/{name}/SKILL.md`

The `agent-generation.skill.md` template is unchanged (still writes to `.ryo/agents/`).

The `ryo-gen.skill.md` orchestrator adds a final step: "Run `npx ryo-kit sync` to link generated agents and skills to your coding tools."

### Changes to `scaffoldProjectDir()`

- Stops creating `.ryo/skills/` directory
- Creates `.agents/skills/` directory instead
- `.ryo/agents/`, `.ryo/workflows/`, `.ryo/.state/`, `.ryo/.customize/` unchanged

### Copilot Runtime Overhaul

The `CopilotRuntime` class changes significantly:
- `skillsDir` changes from `.github/prompts` to `.github/skills`
- `installSkill()` creates directory symlinks instead of writing flat `.prompt.md` files
- New `installAgent()` method creates file symlinks in `.github/agents/`
- `uninstall()` removes symlinks instead of deleting content files
- `configFile` stays as `.github/copilot-instructions.md`

### Directory Structure (After)

```
project/
├── .ryo/
│   ├── agents/                         # Canonical agents
│   │   ├── builder.agent.md
│   │   └── reviewer.agent.md
│   ├── workflows/
│   ├── .state/                         # gitignored
│   └── .customize/
├── .agents/
│   └── skills/                         # Canonical skills
│       ├── ryo-gen/SKILL.md            # Bootstrap
│       ├── ryo-help/SKILL.md           # Core
│       ├── implement/SKILL.md          # Generated
│       └── review/SKILL.md             # Generated
├── .claude/
│   ├── skills/
│   │   ├── ryo-gen → ../../.agents/skills/ryo-gen           # symlink
│   │   └── implement → ../../.agents/skills/implement       # symlink
│   └── agents/
│       └── builder.md → ../../.ryo/agents/builder.agent.md  # symlink
├── .github/
│   ├── skills/
│   │   ├── ryo-gen → ../../.agents/skills/ryo-gen           # symlink
│   │   └── implement → ../../.agents/skills/implement       # symlink
│   ├── agents/
│   │   └── builder.agent.md → ../../.ryo/agents/builder.agent.md  # symlink
│   ├── copilot-instructions.md
│   └── workflows/                      # CI/CD, unchanged
├── .codex/
│   └── agents/
│       └── builder.toml                # Generated (TOML transform)
├── .windsurf/
│   └── rules/
│       ├── ryo-gen.md                  # Copy with trigger: frontmatter
│       └── implement.md                # Copy with trigger: frontmatter
├── AGENTS.md                           # Agent block for Cursor/Windsurf
└── GEMINI.md                           # Agent block for Gemini CLI
```

### Git Considerations

- Symlinks are committed to git (stored as symlink entries)
- `.ryo/.state/` remains gitignored
- `.codex/agents/*.toml` and `.windsurf/rules/*.md` are committed (generated but deterministic)
- `AGENTS.md` and `GEMINI.md` use ryo-kit sentinel blocks (`<!-- ryo-kit:start -->` / `<!-- ryo-kit:end -->`) to avoid clobbering user content

### Conflict Detection

Before creating `.agents/skills/`, `ryo sync` and `ryo gen` check whether `.agents/` already exists and was not created by ryo-kit. If `.agents/` exists but does not contain a `.ryo-kit` marker file, warn the user and abort unless `--force` is passed. The marker file (`.agents/.ryo-kit`) is a zero-byte file created alongside the directory.

### Migration

Since ryo-kit is pre-1.0 (v0.1.0), breaking changes are acceptable.

`ryo sync` detects and migrates the old layout automatically:
1. If `.ryo/skills/` exists and `.agents/skills/` doesn't → move skills to `.agents/skills/`
2. If `.github/prompts/ryo-*.prompt.md` exists → remove old files, create new symlinks in `.github/skills/`
3. If `.windsurfrules` contains ryo-kit blocks → migrate to `.windsurf/rules/*.md` files, remove blocks from `.windsurfrules`
4. If `skills/ryo-*` directories exist at project root (old Codex layout) → remove them (Codex now reads from `.agents/skills/`)
5. If runtime directories contain real files where symlinks are expected → replace with symlinks
5. Print a summary of what was migrated

`ryo gen` on existing projects detects old layout and runs migration before installing new skills.

### Windows Copy Fallback and Git

When symlinks fall back to copies on Windows, those copies are real files that duplicate canonical content. These copies should still be committed to git (for portability on mixed-platform teams). `ryo sync` is idempotent — running it again overwrites copies with fresh content from canonical files. The CLI warns users on copy fallback to re-run `ryo sync` after editing canonical files.

## Scope of Changes

| File | Change |
|---|---|
| `src/runtimes/base.js` | Add `installAgent()`, `agentsDir` to interface |
| `src/runtimes/copilot.js` | Rewrite: symlinks to `.github/skills/` + `.github/agents/` |
| `src/runtimes/claude-code.js` | Change `installSkill` to create symlinks; add `installAgent` |
| `src/runtimes/codex.js` | Change `installSkill` to auto-discovery (no-op); add `installAgent` (TOML gen) |
| `src/runtimes/cursor.js` | Change `installSkill` to auto-discovery (no-op); add `installAgent` (config block) |
| `src/runtimes/gemini-cli.js` | Change `installSkill` to auto-discovery (no-op); add `installAgent` (config block) |
| `src/runtimes/windsurf.js` | Change `installSkill` to copy with transform; add `installAgent` (config block) |
| `src/scaffolder/directory.js` | Create `.agents/skills/` instead of `.ryo/skills/` |
| `src/scaffolder/skill-writer.js` | Write to `.agents/skills/`; call sync after install |
| `src/cli/commands/sync.js` | New file: `ryo sync` command |
| `src/cli/index.js` | Register `sync` command |
| `src/utils/symlink.js` | New file: cross-platform symlink helpers with fallback |
| `templates/bootstrap/ryo-gen.skill.md` | Update `.ryo/skills/` references to `.agents/skills/`; add final step: run `npx ryo-kit sync` |
| `templates/sub-skills/skill-generation.skill.md` | Change output path to `.agents/skills/` |
| `templates/core-skills/ryo-add-skill.skill.md` | Update `.ryo/skills/` references to `.agents/skills/` |
| `templates/core-skills/ryo-evolve.skill.md` | Update `.ryo/skills/` references to `.agents/skills/` |
| `templates/core-skills/ryo-help.skill.md` | Update `.ryo/skills/` references to `.agents/skills/` |
| `test/runtimes.test.js` | Update for symlink behavior |
| `test/scaffolder.test.js` | Update for new directory structure |
| New: `test/sync.test.js` | Tests for sync command |
