...
**Decision and outcome (2026-09-03).** Rather than detecting superpowers and generating overlays for it (6.2), ryo-kit borrows the mechanisms and makes them its own, driven by the org's policy: a structured constitution (6.1), `ryo classify` with path-based scope overrides and the one-way ratchet (6.3), governance-grade gates with `ryo check` enforcement (6.6), a ledger with rulings and audit retention plus new signal types feeding `/ryo-retro` (6.5, expressed natively rather than by importing SDD ledgers), a SessionStart hook and `/ryo-session` bootstrap (6.4, session-start half; 6.8's plugin packaging deferred), rationalization tables and structural requirements for the generated `plan`/`implement`/`review` skills (6.9), and the housekeeping fixes in section 5. A second pass added the PreToolUse / beforeShellExecution guard hook with a compiled policy and staleness check (6.4, enforcement half), the never-called runtime instructions block, a ledger parser with `ryo check` validation, `GATE_WEAKENED`, and hotfix handling in `ryo classify`. A third pass added `ryo trace` (6.7), which also covers what 6.4 called `ryo check --gates`: commits are mapped to ledger steps, and gates passed without evidence are reported. Still open, deliberately: plugin packaging (6.8) — ryo-kit's value is per-org generated content plus project hooks, which a static plugin bundle does not carry; revisit if the meta-skills alone prove worth distributing that way. A behaviour-probe harness for the templates (6.9) needs an LLM in the loop and does not belong in the zero-dependency CLI; the right home is an opt-in `test/behaviour/` suite run manually, which is not yet written.
