...
- **Structured constitution** — `constitution.md` gains YAML frontmatter with machine-checkable rules: `protected_branches`, `required_reviewers`, `forbidden_paths`, `stop_conditions`, `scope_overrides`, `evidence`, `audit`. Validated by `ryo check` via the new `ConstitutionSchema`. `ryo init` tunes the defaults to the org context (solo: no protected branches; compliance or large teams: two reviewers; compliance: checklist evidence), preserving the template's comments.
...
- **Guard hook** — `.ryo/hooks/guard.js` enforces the constitution at tool-call time: refuses pushes to, merges into, and deletions of `protected_branches`; refuses `gh pr merge` / `glab mr merge`; refuses edits and shell writes to `forbidden_paths`. Registered as a Claude Code `PreToolUse` hook (`Bash|Edit|Write|MultiEdit|NotebookEdit`) and a Cursor `beforeShellExecution` hook. Reads `.ryo/hooks/policy.json`, compiled by `ryo sync` from the constitution; `ryo check` reports the policy as stale when the constitution has changed.
- **Runtime instructions block** — `ryo sync` now writes the managed `ryo-kit` block into `CLAUDE.md`, `AGENTS.md`, `GEMINI.md`, `.github/copilot-instructions.md`, and `.cursorrules`, pointing at `.ryo/` and `/ryo-session`. This is the bootstrap for runtimes without hooks. (`updateConfig` existed but was never called.)
- **Ledger parser** — `parseLedger` classifies ledger entries; `ryo check` validates the identity line, entry shapes, and `Ruling:` format.
- `ryo classify --hotfix` (or `--scope hotfix`): emergency path flag, orthogonal to size; overrides and unskippable gates still apply.
- `ryo update` refreshes installed hook scripts in `.ryo/hooks/`.
- **`ryo trace`** — traceability report: maps every commit on the branch to the ledger step and gate that produced it (live ledger plus retained audit ledgers), flags untraced commits, gates passed without evidence, and commit ranges that do not exist. `--strict` for CI, `--json` for tooling.
...
- **`ryo check` rules** — validates the org-wide constitution when the repo has none; workflow steps must reference real process phases; `handoff_to` must reference real agents; process phase agents must exist; scale rules may only skip phases/steps their gate's `skippable_for` allows; automated gates cannot claim separation of duties or approver roles; a performer cannot approve its own gate; a step gate cannot weaken its phase gate; constitution frontmatter, `signals.md` entries, `ledger.md`, and guard policy freshness are validated.
